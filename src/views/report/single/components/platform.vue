...
<script>
import {platformSingleData} from '@/api/customerStatement'
       ...
       this.request.page = val;
        ...
        exportReport(){
            if(!this.tableData.length){
             ...
             let url =`http://wearewwx.com:8001/customerStatement/odpd?queryDate=${this.request.queryDate}`;
             downFile(url)
        },
        dataChange(value){
            if(value.length){
      ...
      async  getData(){
          try {
           this.loading = true;
            ...
            let res = await platformSingleData(this.request)
             let all = 0;
            let validAll = 0;
            res.data.map(item =>{
